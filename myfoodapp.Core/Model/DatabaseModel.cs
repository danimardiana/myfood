...
namespace myfoodapp.Core.Model
{
    public class DatabaseModel
    {
        ...
        private DatabaseModel()
        {
            ...
            var bol = db.Database.EnsureCreated();
            var bil = db.SensorTypes.Count() == 0;

            if(db.SensorTypes.Count() == 0)
            {
                db.SensorTypes.AddAsync(new SensorType(){ Id = 1, name = "pH"});
                db.SensorTypes.AddAsync(new SensorType(){ Id = 2, name = "waterTemperature"});
                db.SensorTypes.AddAsync(new SensorType(){ Id = 3, name = "dissolvedOxygen"});
                db.SensorTypes.AddAsync(new SensorType(){ Id = 4, name = "EC"});
                db.SensorTypes.AddAsync(new SensorType(){ Id = 5, name = "airTemperature"});
                db.SensorTypes.AddAsync(new SensorType(){ Id = 6, name = "ORP"});
